...
const VirtualCard: React.FC<VirtualCardProps> = ({ accountNumber, holderName, type, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 sm:p-6">
            <div className="max-h-[calc(100vh-2rem)] overflow-y-auto bg-white rounded-3xl p-5 sm:p-8 max-w-lg w-full shadow-2xl animate-in fade-in zoom-in duration-300">
                <div className="flex justify-between items-center gap-4 mb-6 sm:mb-8">
                    <h3 className="text-xl sm:text-2xl font-black text-slate-900">Tu Tarjeta Virtual</h3>
                    <button onClick={onClose} className="text-slate-400 hover:text-slate-600 font-bold">Cerrar</button>
                </div>

                <div className={`relative w-full aspect-[1.58/1] rounded-2xl p-4 sm:p-8 text-white shadow-2xl overflow-hidden ${
                    ...
                    <div className="flex justify-between items-start mb-5 sm:mb-12">
                        <div className="w-9 h-7 sm:w-12 sm:h-10 bg-yellow-400/80 rounded-lg border border-yellow-200/50 shadow-inner">
                            <div className="w-full h-full opacity-20 bg-[radial-gradient(circle, #000 1px, transparent 1px)] bg-size-[4px_4px]"></div>
                        </div>
                        <CreditCard className="w-7 h-7 sm:w-10 sm:h-10 opacity-50" />
                    </div>

                    <div className="mb-4 sm:mb-8">
                        <p className="text-[9px] sm:text-xs uppercase tracking-widest opacity-60 mb-1 sm:mb-2">Número de cuenta</p>
                        <p className="text-sm sm:text-2xl font-mono font-bold tracking-wider sm:tracking-[0.2em] break-all">{formattedNumber}</p>
                    </div>

                    <div className="flex justify-between items-end">
                        <div className="min-w-0 pr-3">
                            <p className="text-[10px] uppercase tracking-widest opacity-60 mb-1">Titular</p>
                            <p className="font-bold text-xs sm:text-lg uppercase tracking-tight truncate">{holderName}</p>
                        </div>
                    ...
                    <Wifi className="absolute top-1/2 right-4 sm:right-8 w-5 h-5 sm:w-6 sm:h-6 rotate-90 opacity-40" />
                </div>
